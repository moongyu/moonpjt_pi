...
#
# 191124 /w change force ch -> by detecting voltage
#        /w add log (at wakeup time/1day, shutdown event)
#        /w charge fet on over wakeup time
#        /w charge fet off under wakeup time
# 200529 forced charge fixed (floating charge)
####################################
import blynklib
import blynktimer
...
#event_time_sleep = nownow_time.replace(hour=0, minute=0, microsecond=0)
#event_time_wakeup = now.replace(hour=6, minute=0, microsecond=0)

# no use event_time_sleep
...
event_time_wakeup = datetime.datetime(2019,10,3,2,00,0)
...
def detect_lowvoltage_task() :
	...
	if batlow_f == 1 :
            ...
            if batlowcnt >= 3 :
                file = open('log_mobile.txt','a')
                log_cnt = log_cnt + 1
                file.write(str(log_cnt) + ' : ' + 'shutdown ' + now_str + '\n')
                file.close()
...
def parking_task_5():
    ...
    # charge method : by timer&forcech
    #det_packvol()
...
# no use ctrl_charge_en(), forced_ch
def ctrl_charge_en() :
    if detect_packvol == 1 or forced_ch == 1:
        if now_time >= event_time_sleep and now_time < event_time_wakeup :
            # off
            #GPIO.output(GPIO_OUT_CHARGE_EN, False) # 200529 forced ch
            #blynk.virtual_write(13, 0)   # Vpin =  V13
            
            # on
            GPIO.output(GPIO_OUT_CHARGE_EN, True) # 200529 forced ch 
...
# charge fet on at event_time
def ctrl_charge_by_detectvol():
    if detect_packvol == 1:
...
def start_timer_500ms():
    ...
    if exit_th == 1 :
        timer_500ms=threading.Timer(3,start_timer_500ms) # 3sec.
        timer_500ms.start()

        # update now time (per 500ms)
        now = datetime.datetime.now()
        now_time = now.time()
        now_str = now.strftime('%H %M %S')
...
try:
    ...
    while True:
            ...
            # charge method : by detect voltage           
            det_packvol()
            ctrl_charge_by_detectvol()
